...
@section('pageTitle', 'アナウンスを編集')

@section('content')
	<div class="l-container l-container-narrow">
    <form id="editForm" class="needs-validation py-2 mb-4" method="POST" action="{{ route('admin.notifications.update', $notification) }}" enctype="multipart/form-data" novalidate>
      @csrf
      {{ method_field('PATCH') }}

	    <h2 class="py-4 text-center alt-font">アナウンスを編集</h2>
      <div class="form-group pb-3 row">
	      <label for="formTitle" class="col-4 col-form-label">タイトル</label>
	      <div class="col-8">
	        <input type="text" class="form-control" id="formTitle" name="title" value="{{ old('title', $notification->title) }}" placeholder="" required>
      ...
      <div class="form-group pb-3 row">
	      ...
	      <div class="col-8">
          <textarea class="form-control" id="formContent" name="description" placeholder="" rows="4" style="min-height: 100px;" required>{{ old('description', $notification->description) }}</textarea>
          <div class="invalid-tooltip">
      ...
      <div class="form-group pb-3 row">
          ...
          <select class="custom-select" id="formTarget" name="target_id" required>
            ...
            @foreach($targets as $id => $target)
              <option value="{{ $id }}" {{ old('target_id', $notification->target_id) == $id ? 'selected' : '' }}>{{ ucwords($target) }}</option>
            @endforeach
          </select>
	        <div class="invalid-tooltip">
      ...
      <div class="form-group pb-3 row">
          ...
          <div class="input-group">
            <input type="text" class="form-control js-datepicker" id="formStaffBirthdate" name="published_at" value="{{ old('published_at', $notification->published_at->format('Y-m-d')) }}" placeholder="" required>
      ...
      <div class="form-group pb-3 row">
	      ...
	      <div class="col-8">
            ...
            @foreach($genres as $id => $genre)
              <option value="{{ $id }}" {{ old('genre_id', $notification->genre_id) == $id ? 'selected' : '' }}>{{ ucwords($genre) }}</option>
            @endforeach
	        </select>
	        <div class="invalid-tooltip">

      ...
      <tr>
        ...
        <td>
          ...
          <dl>
            ...
            <dd>{{ $work->role ?? '--' }}</dd>
            @if ($work->started_at->format('Y年m月d日'))
              <dt class="text-muted">在籍期間</dt>
              <dd>{{ $work->started_at->format('Y年m月d日') }}{{ $work->ended_at ? ' - ' . $work->ended_at->format('Y年m月d日') : '- present' }}</dd>
            @endif
        ...
        <td>
          @foreach($student->education_history as $index => $education)
            <dl>
              ...
              <dd>{{ ucwords($education->faculty) ?? '--' }}, {{ ucwords($education->major) ?? '--' }}, {{ ucwords($education->department) ?? '--' }}</dd>
              @if ($education->graduated_at)
                <dt class="text-muted">卒業</dt>
                <dd>{{ $education->graduated_at->format('Y年m月d日') }}</dd>
              @endif
